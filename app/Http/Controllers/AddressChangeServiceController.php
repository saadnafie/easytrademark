...
/**
 * Class AddressChangeServiceController
 * @package App\Http\Controllers
 * @author Andrew Nady <andro.nady2015@gmail.com>
 */
class AddressChangeServiceController extends Controller
{
    ...
    /**
     * show address service form ( stepper )
     *
     * @param $packageID
     * @param $serviceId
     * @param $countryId
    ...
    public function stepper($packageID, $serviceId, $countryId, $countryPackageFees, Request $request)
    {
        ...
        if ($nextPackage == null) {
            return view('client.services.addressChange', $returnData);
        } else {
            $nextPackage = Service::find($serviceId)->package->where('flag', $nextFlag)->first();
    ...
    /**
     * store address change service on DB
     *
     * @param Request $request
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'newAddress' => ['required', 'string', 'different:oldAddress'],
        ...
        // store in table TrademarkAddressChange
        $address = new TrademarkAddressChange;
        $address->old_address = $request->oldAddress;
        $address->new_address = $request->newAddress;
        $address->status = 0;
        ...
        if (Auth::check() == true) {
            ...
            if (App::environment('production')) {
                ...
                $email->old_address = $request->oldAddress;
                $email->new_address = $request->newAddress;
            ...
            session()->put('intendedUrl', route('store-address-change-order-after-login'));
            return redirect()->route('forceUserLogin');
        }
    }

    public function storeAddressChangeOrderAfterLogin (Request $request) {
        ...
        if (App::environment('production')) {
            ...
            \Illuminate\Support\Facades\Mail::send('client.emails.serviceMail', ['email' => $email], function ($message) {
                $message->from('info@easy-trademarks.com', 'Easytrademark');
                $message->to(auth()->user()->email, auth()->user()->name)->subject('ACTION REQURED  ! ');
            });
        }
