...
/**
 * Class RenewalServiceController
 * @package App\Http\Controllers
 * @author Andrew Nady <andro.nady2015@gmail.com>
 */
class RenewalServiceController extends Controller
{
    ...
    /**
     * show renewal service form ( stepper )
     *
     * @param $packageID
     * @param $serviceId
     * @param $countryId
    ...
    public function stepper($packageID, $serviceId, $countryId, $countryPackageFees, Request $request)
    {
        ...
        $returnData = [
            ...
            'serviceId' => $serviceId,
            'quickTurnaroundCost' => $quickTurnaroundCost,
        ...
        if ($nextPackage == null) {
            return view('client.services.renewal', $returnData);
        } else {
            $nextPackage = Service::find($serviceId)->package->where('flag', $nextFlag)->first();
    ...
    /**
     * store Renewal service data in DB
     *
     * @param Request $request
    ...
    public function store(Request $request)
    {
        ...
        // select service based on service which seleted in the begining of the process ( search box ( country - service ))
        ...
        if (Auth::check() == true) {
            ...
            session()->put('intendedUrl', route('store-renewal-order-after-login'));
            return redirect()->route('forceUserLogin');
        }
    }

    public function storeRenewalOrderAfterLogin (Request $request) {
        ...
        if (App::environment('production')) {
            ...
            $email->country_name = $countryName->country_name_en;
            $email->filling_number = $filling['fillingNumber'];
            $email->filling_date = $filling['fillingDate'];
            $email->total_fees_currency = $order->total_fees_currency;
            $email->userCountryCurrencyCode = $userCountryCurrencyCode;

            $SUBJECT = 'ACTION REQURED - order confirmation - '.$order->id.' - '.$trademark['trademark_reference'].' - '.$trademark['trademark_label'].' - '.$countryName->country_name_en;
